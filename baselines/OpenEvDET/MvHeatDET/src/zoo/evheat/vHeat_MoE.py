...
class Heat2D(nn.Module):
    ...
    def haar_transform_1d(self, x):
        ...
        # Haar 变换通过均值和差值构建
        avg = (x[..., ::2] + x[..., 1::2]) / 2  # 均值成分
        diff = (x[..., ::2] - x[..., 1::2]) / 2  # 差值成分
        return torch.cat((avg, diff), dim=-1)
    
    def haar_transform(self, x, dims=(-2, -1)):
        transformed = x.clone()  # 创建副本以保存变换结果
        for dim in dims:
            # 将每个指定维度上的哈尔变换应用到所有其他维度上
            shape = transformed.shape
            transformed = transformed.view(-1, shape[dim])  # 将指定维度展开到最后
            transformed = self.haar_transform_1d(transformed)  # 在该维度上应用哈尔变换
            transformed = transformed.view(*shape)  # 恢复原始形状
        return transformed
    ...
    def inverse_haar_transform_1d(self, x):
        ...
        # 将输入分为前半部分和后半部分，分别对应均值和差值
        avg = x[..., :n // 2]
        diff = x[..., n // 2:]
        
        # 恢复原始数据
        x_rec = torch.zeros(x.shape[:-1] + (n,), device=x.device)
        x_rec[..., ::2] = avg + diff  # 重构偶数索引
        x_rec[..., 1::2] = avg - diff  # 重构奇数索引
        return x_rec
    ...
    def forward(self, x: torch.Tensor, freq_embed=None):
        ...
        x_dct = F.conv1d(x.contiguous().view(B, H, -1), weight_cosn.contiguous().view(N, H, 1))
        x_dct = F.conv1d(x_dct.contiguous().view(-1, W, C), weight_cosm.contiguous().view(M, W, 1)).contiguous().view(B, N, M, -1)
        ...
        if self.infer_mode:
            ...
            x_dct = torch.einsum("bnmc,nmc -> bnmc", x_dct, weight_exp) # exp decay
            x_fft = torch.einsum("bnmc,nmc -> bnmc", x_fft, weight_exp) # exp decay
            x_haar = torch.einsum("bnmc,nmc -> bnmc", x_haar, weight_exp) # exp decay
        
        x_dct = F.conv1d(x_dct.contiguous().view(B, N, -1), weight_cosn.t().contiguous().view(H, N, 1))
        x_dct = F.conv1d(x_dct.contiguous().view(-1, M, C), weight_cosm.t().contiguous().view(W, M, 1)).contiguous().view(B, H, W, -1)

        x_fft = torch.fft.ifftn(x_fft).real

        x_haar = self.inverse_haar_transform(x_haar)
        # two moe
        # x = torch.cat((x_fft.unsqueeze(dim=1), x_haar.unsqueeze(dim=1)), dim=1) 

        x = torch.cat((x_dct.unsqueeze(dim=1), x_fft.unsqueeze(dim=1), x_haar.unsqueeze(dim=1)), dim=1)
...
if __name__ == "__main__":
    ...
    model = vHeat_MoE().cuda()
    input = torch.randn((2, 3, 224, 224), device=torch.device('cuda'))
